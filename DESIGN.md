...
7. **Mechanically verified parity:** cloned `claude-agent-sdk-python` and diffed `__all__` (123) + `ClaudeAgentOptions` fields (45) against the Go surface; closed every gap — thinking-config union, `MirrorErrorMessage`, `rate_limit_event` camelCase decode, session first-prompt filtering (`<local-command-stdout>` etc.) + `<command-name>`, `FoldSessionSummary`, `ImportSessionToStore`, 11 missing options (`--tools`, `--session-id`, `--strict-mcp-config`, `--include-hook-events`, `--effort`, `--task-budget`, max-buffer, load-timeout, file-checkpointing, run-as-user, live `WithSessionStore` mirror). 123/123 names and 45/45 options covered (handful documented N/A).
8. **Behavioral verification + re-sync to CLI 2.1.159:** added an integration suite that runs the real `claude` binary, which caught bugs static parity missed — a one-shot `Query` stdin hang (now closes stdin after the prompt, mirroring `wait_for_result_and_end_input`) and a dead `CanUseTool` (now emits `--permission-prompt-tool stdio`). Re-checking against 2.1.159 fixed the `--thinking` value emission (typed `ThinkingConfig` → `--thinking adaptive`/`--max-thinking-tokens N`/`--thinking disabled` + `--thinking-display`). Added `CLAUDE.md`, `CHANGELOG.md`, and 16 examples mapped to upstream. Lesson: static parity (names/fields/values) is necessary but not sufficient — only the live binary verifies behavior.
